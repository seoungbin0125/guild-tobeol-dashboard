...
전투력/토벌전 현황, 캐릭터 모아보기, 버츄얼 광장, 길드 컨텐츠 매칭, 기프트카드 핫딜, 실시간 공략 게시판, 주요 링크, 쉬어가기 게임 링크를 한 화면에서 볼 수 있는 GitHub Pages용 사이트입니다.
...
| 핫딜 | 아카라이브 핫딜 게시판의 기프트카드 할인 후보 최신 5개 |
| 공략 | 공략 게시판 작성/조회 |
| 링크 | 디스코드, 메키 단톡방 |
| 쉬어가기 | 게임 1, 게임 2 |
...
핫딜 탭은 아카라이브 핫딜 게시판에서 `기프트카드` 키워드로 검색한 글을 `data/hotdeals.json`에 저장한 뒤 보여줍니다.

- 제목에 `종료`, `마감`, `품절` 같은 문구가 있으면 `마감 가능`으로 표시합니다.
- 종료 문구가 없고 상품권/기프트카드 관련 키워드가 있으면 `유효 가능`으로 표시합니다.
- 유효 여부를 확정하기 어려우면 최신 5개 후보를 보여줍니다.

최신 핫딜 데이터를 수동으로 갱신하려면 아래 명령을 실행합니다.

```bash
npm run collect:hotdeals
```

GitHub Actions에는 `.github/workflows/collect-hotdeals.yml`이 추가되어 있어 매시간 자동으로 `data/hotdeals.json`을 갱신합니다.
...
MGF 길드 데이터 GitHub Actions는 매일 KST 00:10에 자동 실행되도록 설정되어 있습니다. 기프트카드 핫딜 Actions는 매시간 실행됩니다.
...
- name: Collect data
  env:
    ...
    TOBEOL_MAX_PAGE: "10"
    MEMBER_LIMIT: "100"
  run: node scripts/collect-mgf.js
```
...
수집 스크립트의 기본 길드원 제한을 30명에서 100명으로 올렸습니다. 새로 가입한 길드원이 낮은 순위라 30위 밖에 있어도 `data/latest.json`에 들어오도록 수정했습니다.

수집 결과에는 아래 값도 포함됩니다.

- `memberStatus: "new"`: 7일 전 스냅샷에는 없고 현재 길드원 목록에는 있는 신규/복귀 멤버
- `memberChanges`: 길드별 신규/탈퇴 인원 요약
- `departedMembers`: 7일 전에는 있었지만 현재 목록에는 없는 멤버

현황 화면에는 `신규/탈퇴` 요약 카드가 표시되고, 신규 멤버 닉네임에는 `신규` 배지가 붙습니다.

## v2.4 광장 조작 개선

- 광장을 작은 보드판형 UI에서 가로로 긴 2D 사이드뷰 맵으로 변경했습니다.
- 모바일에서는 맵 영역을 좌우로 스크롤하면서 이동할 수 있고, 내 캐릭터 위치로 자동 스크롤됩니다.
- 공격/돌진/회복/실드 스킬 버튼을 모바일 화면 하단 플로팅 버튼으로 분리했습니다.
- 광장 입장 전에는 다른 캐릭터가 먼저 보이지 않고, 입장 안내 카드만 표시됩니다.
- 공격/스킬 기록은 채팅창에 올라가지 않습니다. 채팅은 실제 대화 메시지만 표시됩니다.
- 실시간 멀티 광장 사용 시 `shieldUntil` 필드가 추가되었으므로 `firestore.rules`를 다시 Publish 해주세요.
